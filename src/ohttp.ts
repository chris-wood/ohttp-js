...
function encodeSymmetricAlgorithms(kdf: Kdf, aead: Aead) {
  return [0x04, (kdf >> 8) & 0xFF, kdf & 0xFF, (aead >> 8) & 0xFF, aead & 0xFF];
}
...
export class Server {
  ...
  async encodeKeyConfig(): Promise<Uint8Array> {
    ...
    const encodedKey = await this.suite.serializePublicKey(
...
export class ClientConstructor {
  async clientForConfig(config: Uint8Array): Promise<Client> {
    ...
    const publicKey = await suite.deserializePublicKey(
      config.slice(3, 3 + suite.publicKeySize),
    );
    const offset = 3 + suite.publicKeySize + 2; // skip over the length, since we pick the first one
